...
router.put('/:id', authenticate, authorize('admin', 'funcionario'), validate(tagSchemas.updateTagSchema), auditLog('tag', 'update'), tagController.updateTag);

// Protected routes (admin only)
router.delete('/:id', authenticate, authorize('admin'), validate(tagSchemas.deleteTagSchema), auditLog('tag', 'delete'), tagController.deleteTag);
